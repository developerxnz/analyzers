...
/// </summary>
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(RequestCodeFixProvider)), Shared]
public class RequestCodeFixProvider : CodeFixProvider
{
    private const string CommonName = "Request";

    // Specify the diagnostic IDs of analyzers that are expected to be linked.
    public sealed override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create(RequestSyntaxAnalyzer.DiagnosticId);
        ...
        ClassDeclarationSyntax classDeclarationSyntax, CancellationToken cancellationToken)
    {
        ...
        var newName = classDeclarationSyntax.Identifier.Text + "Request"; //Replace(SampleSyntaxAnalyzer.CompanyName, CommonName);
